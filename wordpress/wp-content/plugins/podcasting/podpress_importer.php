...
<?php
	...
	function import_podcasts($podcasts) {
		...
		foreach ( $podcasts AS $podcast ) {
			...
			if ( !$no_enclose ) {
				...
				$headers = podcasting_get_http_headers($content);
	...
	function PodPress_Import() {
		// Nothing.
	}
}

$podPress_import = new PodPress_Import();

$wp_importers['podpress'] = array('PodPress', 'Import podPress podcasts into Podcasting.', array ($podPress_import, 'dispatch'));

//register_importer('podpress', __('PodPress'), __('Import podPress podcasts into Podcasting.'), array ($podPress_import, 'dispatch'));
?>